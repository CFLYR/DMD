...
def test_variant(variant_name, dataset_name, config_dir="experiments/ablation_study/configs",
    ...
    try:
        ...
        # Format: {'Acc_7': x, 'Acc_2': y, 'F1_score': z, 'Loss': w}
        print(f"  ✓ Test completed successfully!")
        if results:
            print(f"    ACC7: {results.get('Acc_7', 'N/A')}")
            print(f"    ACC2: {results.get('Acc_2', 'N/A')}")
    ...
    for variant_info in ABLATION_VARIANTS:
        ...
        for dataset in DATASETS:
            ...
            if key in all_results and all_results[key]:
                res = all_results[key]
                row[f"{dataset.upper()}_ACC7"] = f"{res.get('Acc_7', 0.0):.2f}"
                row[f"{dataset.upper()}_ACC2"] = f"{res.get('Acc_2', 0.0):.2f}"
                row[f"{dataset.upper()}_F1"] = f"{res.get('F1_score', 0.0):.2f}"
            else:
